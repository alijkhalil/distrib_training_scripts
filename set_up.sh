...
VID_PROCESS_REPO_NAME="video_processing_networks"
...
# Do video processing
if [ ! -d ../$VID_PROCESS_REPO_NAME ]; then
    if [ -d $VID_PROCESS_REPO_NAME ]; then
        rm -rf $VID_PROCESS_REPO_NAME
    fi

    git clone https://github.com/alijkhalil/$VID_PROCESS_REPO_NAME.git > /dev/null 2>&1
    if [ $? -ne 0 ]; then
        echo "Download error!" >&2
        exit 1
    fi

    mv $VID_PROCESS_REPO_NAME ../
fi
